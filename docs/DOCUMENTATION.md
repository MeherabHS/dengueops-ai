...
| Operational directives | Rule-based, tiered | Action recommendations |
...
Dengue transmission has a biological delay chain:
...
Total lag: ~14 days minimum, ~28 days for population-level signal.
...
| Growth Factor | Risk Score Range | Risk Level |
|--------------|-----------------|------------|
| ≤ 1.0 | 0–20 | Low |
...
```
Best Case     = max(0, Forecast − RMSE)
Expected Case = Forecast
Worst Case    = Forecast + RMSE
```
...
|----------|---------------|---------------|------------|
| Best Case | 166 | 1.063 | Low |
| Expected Case | 234 | 1.498 | Moderate |
| Worst Case | 302 | 1.933 | High |
...
```
Priority Score = Forecast Risk Score × (1 + Vulnerability Weight)
```
...
|---------|----------------|
| Priority Score ≥ 76 or Bed Gap > 0 | ACTIVATE |
| Priority Score 51–75 or SDH < 7 days | ALERT |
| Priority Score 26–50 | WATCH |
...
| Technical Tab | Model evidence + feature importance + pipeline status |
...
|------|----------|----------|---------|
| `dengue_cases.csv` | `fetch_opendengue.py` | `feature_engineering.py` | **Real** OpenDengue Bangladesh national dengue cases, weekly (2014–2024). Source: Clarke et al. 2024, *Sci Data*. |
...
| Dengue case counts | **Real** — OpenDengue V1.3 Bangladesh national data (Clarke et al. 2024) |
