...
import { BRAND, SDH_CRITICAL_THRESHOLD, SDH_WARNING_THRESHOLD } from "@/lib/constants";
...
export default function SupplyDepletionChart({ directives }: Props) {
  ...
  const getNs1Color = (v: number) =>
    v <= SDH_CRITICAL_THRESHOLD ? BRAND.alertRed : v <= SDH_WARNING_THRESHOLD ? BRAND.alertOrange : BRAND.cyan;
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <BarChart data={data} margin={{ top: 5, right: 16, left: 0, bottom: 5 }}>
        ...
        <ReferenceLine y={SDH_CRITICAL_THRESHOLD} stroke={BRAND.alertRed} strokeDasharray="4 3" label={{ value: "Critical (7d)", fontSize: 10, fill: BRAND.alertRed }} />
        <ReferenceLine y={SDH_WARNING_THRESHOLD} stroke={BRAND.alertOrange} strokeDasharray="4 3" label={{ value: "Warning (14d)", fontSize: 10, fill: BRAND.alertOrange }} />
        <Bar dataKey="ns1" name="NS1/RDT Kit SDH" radius={[4, 4, 0, 0]}>
          {data.map((d, i) => (
            <Cell key={i} fill={getNs1Color(d.ns1)} />
          ))}
        </Bar>
        <Bar dataKey="iv" name="IV Fluid SDH" fill={BRAND.navyMid} opacity={0.7} radius={[4, 4, 0, 0]} />
      </BarChart>
    </ResponsiveContainer>
  );
}
