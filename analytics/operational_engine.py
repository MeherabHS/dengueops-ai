...
Design principles:
    ...
    2. Vulnerability gates priority, not just raw risk:

       A zone with identical forecast risk but higher vulnerability (informal
       settlement, limited facility access) requires earlier pre-emptive action.
       priority_score = risk_score × (1 + vulnerability_weight) gates the
       urgency of operational decisions by structural inequality, which is a
       key SDH-informed principle of equitable response.
    ...
    3. Spatial exposure allocation is a heuristic:
       ...
       surveillance by ward), this is the best available spatial approximation.
...
def calculate_bed_load(
    ...
    """
    daily_surge = allocated_zone_cases / max(1, horizon_days)
    projected = occupied_dengue_beds + daily_surge * avg_length_of_stay
...
def calculate_priority(
    ...
    """
    Compute vulnerability-gated, exposure-adjusted priority score (0–100).
    ...
    *preparedness* decision-support system, not just an outbreak detector — so
    ...
    structural = (exposure_index * vulnerability_weight * 200.0) + (exposure_index * 80.0)
...
def generate_inventory_alerts(
    ...
    for item in facility_items:
        ...
        threshold = int(item["reorder_threshold_days"])
...
def build_directives(
    ...
    ----------
    forecast : dict
        forecast_output.json content (must include uncertainty_scenarios).
    zones : list[dict]
        zones.json content.
    facilities : list[dict]
    ...
    scenarios_raw = forecast.get("uncertainty_scenarios", {})
    if not scenarios_raw:
        raise ValueError(
            "forecast_output.json is missing uncertainty_scenarios. "
            "Run python analytics/uncertainty_engine.py first."
        )
    ...
    for zone in zones_enriched:
        ...
        for facility in zone_facilities:
            ...
            ns1_threshold = int(ns1_items[0]["reorder_threshold_days"]) if ns1_items else 7
            ivf_threshold = int(ivf_items[0]["reorder_threshold_days"]) if ivf_items else 5
    ...
    return {
        ...
        "notes": [
            ...
            "anchor profiles only. Bed counts, occupancy, stock, and readiness values "
            "are synthetic and do not represent actual facility records.",
            "Outputs are advisory and require human expert review before any operational action.",
            ...
            "Uncertainty ranges are based on RMSE-derived forecast scenarios, "
            "not calibrated probabilistic intervals.",
