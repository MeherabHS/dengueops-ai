...
├── forecast_model.py       # Phase 3b — GBR final model + forecast output
├── uncertainty_engine.py   # Phase 4 — RMSE-derived best/expected/worst scenarios
...
### How RMSE-based uncertainty is estimated

The uncertainty band uses a simple but traceable method:

```
expected_case = forecast_cases              (ML point forecast)
best_case     = max(0, forecast_cases - RMSE)
worst_case    = forecast_cases + RMSE
```
...
This is a **prototype approximation**, not a statistically rigorous forecast interval:
