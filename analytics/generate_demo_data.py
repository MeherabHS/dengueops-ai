...
def main() -> None:
    ...
    # 1. Dengue cases (controlled synthetic — Dhaka South 2024-2026 weekly pattern)
    # Note: run --use-opendengue flag to replace with real OpenDengue national data
    df_cases = generate_dengue_cases()
    write_csv(df_cases, DATA_DIR / "dengue_cases.csv", "dengue_cases.csv")

    # 2. Climate data (synthetic — matches 2024-2026 dengue case period)
    df_climate = generate_climate_data()
    write_csv(df_climate, DATA_DIR / "climate_data.csv", "climate_data.csv")

    # 3. Zones
    zones = generate_zones()
    write_json(zones, DATA_DIR / "zones.json", "zones.json")

    # 4. Facilities
    facilities = generate_facilities()
    write_json(facilities, DATA_DIR / "facilities.json", "facilities.json")

    # 5. Inventory
    inventory = generate_inventory()
    write_json(inventory, DATA_DIR / "inventory.json", "inventory.json")
