...
def add_lag_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Epidemiological rationale for lags:
        The Aedes aegypti mosquito development cycle (egg → larva → pupa → adult)
        takes approximately 7–14 days at typical Dhaka temperatures. Combined with
        the dengue extrinsic incubation period (7–14 days in the vector) and a
        further reporting delay of 5–7 days, climate conditions from 2–4 weeks
        prior are the most relevant predictors of current case counts.

        - 2-week lag (14 days): captures early-stage development effects and
          immediate temperature-driven transmission changes.
        - 4-week lag (28 days): captures the full vector development and
          incubation window, particularly for rainfall-driven larval breeding.
...
def add_seasonality_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Monsoon flag (weeks 22–38):
        Marks the active southwest monsoon period in Dhaka (June–September).
        Dengue larval breeding habitat is at its peak during this window.

    Post-monsoon flag (weeks 38–50):
        Marks the post-monsoon dengue surge risk window (October–December).
        Adult mosquito populations remain high after the monsoon and case
        counts typically peak 2–4 weeks after peak rainfall.
...
def build_features(
    ...
    df[["rainfall_mm", "avg_temp_c", "humidity_pct"]] = (
        df[["rainfall_mm", "avg_temp_c", "humidity_pct"]].round(2)
    )
